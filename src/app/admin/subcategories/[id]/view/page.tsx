...
const ViewSubcategoryPage = ({ params }: Props) => {
  ...
  const fetchData = async () => {
    try {
      const [subcategoryData, subSubcategoriesData] = await Promise.all([
        subcategoryService.getSubcategoryById(params.id),
        subSubcategoryService.getSubSubcategoriesBySubcategory(params.id),
      ]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          href={`/admin/subcategories/${params.id}/edit`}
        ...
        <div className="p-6">
          <div className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">{subcategory.category?.category_name || '-'}</p>
            </div>
            <div>
                  ...
                  <tr key={subSubcategory.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">
                        {subSubcategory.sub_sub_categories_name}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
