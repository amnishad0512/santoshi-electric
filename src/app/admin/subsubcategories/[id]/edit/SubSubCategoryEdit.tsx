...
export default function SubSubCategoryEdit({ id }: { id: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [subsubcategoryResponse, categoriesResponse, subcategoriesResponse] = await Promise.all([
          subsubcategoryService.getSubSubCategoryById(id),
          categoryService.getAllCategories(),
          subcategoryService.getAllSubCategories()
        ]);

        if (!subsubcategoryResponse || !subsubcategoryResponse.data) {
          toast.error('Sub-subcategory not found');
          router.push('/admin/subsubcategories');
          return;
        }

        setFormData(subsubcategoryResponse.data);
  ...
  useEffect(() => {
    if (formData.category_id) {
      ...
      if (!filtered.find(sub => sub.id === formData.sub_category_id)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await subsubcategoryService.updateSubSubCategory(id, {
        sub_sub_category_name: formData.sub_sub_category_name,
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Edit Sub-SubCategory</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {loadingSubmit ? 'Updating...' : 'Update Sub-SubCategory'}
          </button>
        </div>
      </form>
    </div>
