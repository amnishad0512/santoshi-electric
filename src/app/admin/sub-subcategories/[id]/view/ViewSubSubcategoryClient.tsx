...
interface ViewSubSubcategoryClientProps {
  id: string;
}

const ViewSubSubcategoryClient = ({ id }: ViewSubSubcategoryClientProps) => {
  ...
  const fetchData = async () => {
    try {
      const response = await subSubcategoryService.getSubSubCategoryById(id);
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="space-x-2">
          <Link
            href={`/admin/sub-subcategories/${id}/edit`}
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="p-6">
          <div className="space-y-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Subcategory ID</h2>
              <p className="mt-1 text-sm text-gray-500">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">{subSubcategory.sub_sub_category_name}</p>
            </div>
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Created At</h2>
              <p className="mt-1 text-sm text-gray-500">
                {subSubcategory.created_at ? new Date(subSubcategory.created_at).toLocaleDateString() : '-'}
              </p>
            </div>
          </div>
        </div>
...
export default ViewSubSubcategoryClient; 