...
interface EditSubSubcategoryClientProps {
  id: string;
}

const EditSubSubcategoryClient = ({ id }: EditSubSubcategoryClientProps) => {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [categories, setCategories] = useState<Category[]>([]);
  const [subcategories, setSubcategories] = useState<SubCategory[]>([]);
  const [formData, setFormData] = useState<Partial<SubSubCategory>>({
    id: '',
    category_id: '',
    sub_category_id: '',
  ...
  const fetchData = async () => {
    try {
      const [categoriesResponse, subSubcategoryData] = await Promise.all([
...
export default EditSubSubcategoryClient; 